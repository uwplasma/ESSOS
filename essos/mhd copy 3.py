...
class VMECBoundaryJAX:
    ...
    def run_fixed_boundary_for_optimization(self, dofs=None, **kwargs):
        ...
        for i, (ns_i, niter_i, ftol) in enumerate(zip(ns_stages, niter_stages, ftol_stages)):
            ...
            if i == 0:
                bdy_i = boundary
                # Check if R_AXIS and Z_AXIS are present in indata
                r_axis = indata.get('R_AXIS', None)
                z_axis = indata.get('Z_AXIS', None)
                infer_axis = True
                if (r_axis is None or (hasattr(r_axis, '__len__') and all(float(x) == 0.0 for x in r_axis))) and \
                   (z_axis is None or (hasattr(z_axis, '__len__') and all(float(x) == 0.0 for x in z_axis))):
                    infer_axis = False
                st_guess = initial_guess_from_boundary(static_i, bdy_i, indata, vmec_project=vmec_project, infer_axis_if_missing=infer_axis)
            else:
            ...
            import jax.numpy as jnp
            jac_min = jnp.min(g_diag.sqrtg)
            jac_max = jnp.max(g_diag.sqrtg)
            jac_mean = jnp.mean(g_diag.sqrtg)
            print(f"[DIAG] Initial Jacobian min/max/mean: {jac_min} {jac_max} {jac_mean}")
