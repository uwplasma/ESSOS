...
if area_w < 1e-6:
    area_w *= 10000.0
    print(f"area_w corrected to paper scale: {area_w:.4e} m²")
...
G_f32  = np.asarray(compute_G_parallel(dipole_field, surface_pts_flat, surface_nrm_flat), np.float32)
...
def adam_step(pho, m, v, t, lr, w_fB, w_fD):
    """One Adam step: L = w_fB * fB/fB_ref + w_fD * fD."""
    def loss(x):
        bn    = x @ G_jax.T + Bn_jax[None, :]
        fB    = f32(0.5) * jnp.sum(bn * bn, axis=1) * aw_jax
        abs_p = jnp.sqrt(x * x + f32(1e-7))
        fD    = jnp.sum(abs_p * (f32(1) - abs_p), axis=1)
        return jnp.sum(w_fB * fB / fB_ref + w_fD * fD)
