...
class Vmec():
    def __init__(self, wout_filename, ntheta=50, nphi=50, close=True, range_torus='full torus',s_vmec=1.0):
        ...
        self._surface = SurfaceRZFourier(self, ntheta=ntheta, nphi=nphi, close=close, range_torus=range_torus,s=s_vmec)
...
class near_axis_general_frame():
    ...
    def _tree_flatten(self):
        children = (self.rc, self.zs, self.X1c, self.Y1c, self.B0, self.I2, self.B2s, self.B2c)  # arrays / dynamic values
        aux_data = {"p2": self.p2, "nfp": self.nfp, "sG": self.sG, "spsi": self.spsi, "nphi": self.nphi, "sigma0": self.sigma0,
    ...
    def jacobian(self, points):
        ...
        AbsB = self.AbsB(points)
        return r*B0_at_phi*(G0_at_phi+self.iota*self.I2)/(AbsB*AbsB)
...
tree_util.register_pytree_node(near_axis_general_frame,
                               near_axis_general_frame._tree_flatten,
                               near_axis_general_frame._tree_unflatten)