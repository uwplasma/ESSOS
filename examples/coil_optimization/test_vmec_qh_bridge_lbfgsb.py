...
OUTPUT_DIR = Path(__file__).resolve().parent / "output" / "vmec_qh_bridge_lbfgsb"
...
def _scalar_objective_from_vmec(vmec):
    ...
    mean_abs_iota = vmec.mean_abs_iota_from_state(state)
    aspect_term = ASPECT_WEIGHT * (aspect - TARGET_ASPECT) ** 2
    iota_floor_term = IOTA_FLOOR_WEIGHT * smooth_min_abs_iota_residual(
        mean_abs_iota,
        TARGET_ABS_IOTA_MIN,
        softness=SOFTNESS,
    ) ** 2
    return QS_WEIGHT * qs + aspect_term + iota_floor_term
...
def main():
    ...
    print(f"QH bridge jaxopt.LBFGSB test: max_mode={MAX_MODE}, max_nfev={MAX_NFEV}")
    ...
    history_rows = [
        {
            ...
            "iota": float(vmec.iota_from_state(initial_state)),
        ...
        },
        {
            ...
            "aspect": final_aspect,
            "iota": float(opt_vmec.iota_from_state(opt_state)),
    ...
    history = {
        ...
        "label": "ESSOS QH bridge jaxopt.LBFGSB test",
    ...
    print("Final loss:", history["final_loss"])
    ...
    if history["solver_state_iter_num"] is not None:
        print("jaxopt iterations:", history["solver_state_iter_num"])
    if history["solver_state_error"] is not None:
        print("jaxopt reported error:", history["solver_state_error"])
