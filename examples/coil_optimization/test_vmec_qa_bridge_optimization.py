...
OUTPUT_DIR = Path(__file__).resolve().parent / "output" / "vmec_qa_bridge_test"
...
def main():
    vmec = _build_vmec()
    total_loss = custom_loss(_objective_from_vmec, "vmec")
    total_loss.dependencies = {"vmec": vmec}
    ...
    print(f"QA bridge test: max_mode={MAX_MODE}, max_nfev={MAX_NFEV}")
    ...
    t0 = time()
    r0_vec, _J0 = _residual_and_jacobian(starting_dofs)
