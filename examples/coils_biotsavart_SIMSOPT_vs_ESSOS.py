...
nfp_array = [3, 5, 4, 2]
n_curves_array = [3, 7, 6, 4]
curves_array = [get_ncsx_data()[0], get_w7x_data()[0], get_hsx_data()[0], get_giuliani_data()[0]]
currents_array = [get_ncsx_data()[1], get_w7x_data()[1], get_hsx_data()[1], get_giuliani_data()[1]]
name_array = ["NCSX", "W7-X", "HSX", "Giuliani"]

# nfp = 3
# n_curves = 3
# curves_ncsx, currents_ncsx, _ = get_ncsx_data()
for nfp, n_curves, curves_stel, currents_stel, name in zip(nfp_array, n_curves_array, curves_array, currents_array, name_array):
    ...
    R = jnp.mean(jnp.array([jnp.sqrt(coil.curve.x[coil.curve.local_dof_names.index('xc(0)')]**2
        ...
        for coil in coils_simsopt[:n_curves]]))
