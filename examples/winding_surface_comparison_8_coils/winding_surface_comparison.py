...
def sheet_resolution_study(results, cases):
    ...
    figure, axes = plt.subplots(len(cases), 2,
                                figsize=(12, 3.5 * len(cases)),
...
def validate_surfaces(surface_data, cases, resolution=96):
    ...
    figure = plt.figure(figsize=(15, 4 * len(cases)), constrained_layout=True)
    for row_index, case in enumerate(cases):
        for column, surface_method in enumerate(SURFACE_METHODS):
        ...
        for offset, solver in enumerate(METHODS):
            ...
            for position in x[~valid] + (offset - 1) * width:
                axis.text(position, 0.03, "failed", rotation=90, color="#AA0000",
                          ha="center", va="bottom",
                          transform=axis.get_xaxis_transform(), fontsize=7)
        labels = [f"{case.replace('Landreman-Paul ', '')}\n"
                  f"{'offset' if surface == 'normal offset' else 'Pareto'}"
                  for case, surface in rows]
...
def run_comparison():
    ...
    cases = {
        ...
        "Landreman-Paul QH": {
            "wout": os.path.join(
                EXAMPLES, "input_files", "wout_LandremanPaul2021_QH_reactorScale_lowres.nc")},
    ...
    for case, data in cases.items():
        for surface_method in SURFACE_METHODS:
            ...
            for method in solvers:
                ...
                if (case in ("Landreman-Paul QH", "W7-X")
                        and method == "QUADCOIL"
                        and not os.path.exists(result_file)):
                    metrics = dict(
                        method=method, configuration=case,
                        surface_method=surface_method,
                        surface_runtime_s=float(surface_info["runtime"]),
                        surface_iterations=int(surface_info["iterations"]),
                        surface_converged=bool(surface_info["success"]),
                        solver_converged=False, cold_runtime_s=np.nan,
                        steady_runtime_s=np.nan, peak_memory_mb=np.nan,
                        sheet_f_B_T2_m2=np.nan,
                        sheet_max_abs_Bn_over_B=np.nan,
                        achieved_Kmax_A_per_m=np.nan,
                        filament_f_B_T2_m2=np.nan,
                        filament_max_abs_Bn_over_B=np.nan,
                        minimum_distance_m=np.nan,
                        self_intersection_radius_m=np.nan,
                        minimum_jacobian=np.nan,
                        solver_status=("exceeded 6 min and 2.8 GB on QH"
                                       if case == "Landreman-Paul QH" else
                                       "did not converge in the matched study"))
                    np.savez(result_file, metrics=json.dumps(metrics))
