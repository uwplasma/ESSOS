...
def compute_Bn_fixed_essos(coils, surf_pts, surf_n, modB=False):
    ...
    if modB:
        return np.linalg.norm(B_at_pts, axis=-1)
...
# filter for speed ups
arg = jnp.argwhere(pho_loaded != 0)[:, 0]
n_magnets = len(positions)

pho_optimized = pho_loaded
scaled_moments = moments_raw*pho_optimized[:, np.newaxis]

dipole_field = DipoleField(
    positions[arg],
    scaled_moments[arg],
    jnp.zeros(len(positions[arg])),
    nfp=nfp, stellsym=stellsym, scale_factor=1.0,
)

modB_coils = compute_Bn_fixed_essos(essos_coils, surf_pts, surf_n, modB=True)
...
Bn_after  = (Bn_fixed + Bn_pm).reshape(nphi_s, ntheta_s) / (modB_coils.reshape(nphi_s, ntheta_s))


phi_coords   = np.linspace(0, np.pi/nfp, nphi_s)
...
axes[0].set_xlabel(rf"$\phi$"); axes[0].set_ylabel(rf"$\theta$")
axes[0].set_aspect("auto")
cbar = plt.colorbar(im1, ax=axes[0])
cbar.set_label(r"$\frac{B\cdot n}{|B|_{coils}}$", fontsize=20)
...
plt.tight_layout()
plt.savefig(RESULTS_DIR / "Bn_before_after.png", dpi=100, bbox_inches="tight")
plt.close()
