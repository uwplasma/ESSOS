...
for i in range(n_total_optimizations):
    ...
    for n_it, diff_step in zip(n_iteration_least_squares, diff_step_least_squares):
        print(f"Optimization {i+1} of {n_total_optimizations} with {n_it} iterations, ftol={ftol_least_squares}, maxtime={maxtime}, timesteps={timesteps}, diff_step={diff_step}")
        res=optimize(stel, particles, R, r, initial_values, maxtime=maxtime, timesteps=timesteps, method={"method": "least_squares", "ftol": ftol_least_squares, "max_nfev": n_it, "diff_step": diff_step})
    ...
    stel.save_coils(f"output/Optimization_{i+1}.txt", text=f"loss={res}, maxtime={maxtime}, timesteps={timesteps}")
