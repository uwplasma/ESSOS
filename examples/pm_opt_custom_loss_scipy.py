#!/usr/bin/env python3
"""
Full PM optimizer including essos.losses.custom_loss -- SCIPY VERSION.

Identical field/loss setup to pm_opt_custom_loss.py, but replaces the
hand-rolled Adam loop with scipy.optimize.minimize(method='L-BFGS-B'),
which natively supports the pho in [-1,1] box constraint and accepts
our real analytic JAX gradient (via jac=True) instead of falling back
to scipy's slow finite-difference gradient estimate.

Since scipy's minimize runs each call to convergence (unlike Adam's
fixed step count), the two-phase strategy (pure fB minimization, then
annealed discreteness penalty) is implemented as a SEQUENCE of L-BFGS-B
calls: one at wD=0, then several more at increasing wD, each warm-
started from the previous result's solution.
"""
from __future__ import annotations

import sys
import time
from pathlib import Path

import numpy as np
from scipy.optimize import minimize

if len(sys.argv) != 4:
    sys.exit("Usage: python pm_opt_custom_loss_scipy.py <surf_file> <mag_file> <coil_file>")

SURF_FILE = Path(sys.argv[1])
MAG_FILE  = Path(sys.argv[2])
COIL_FILE = Path(sys.argv[3])

SURFACE_RANGE  = "half period"
SURFACE_NPHI   = 64
SURFACE_NTHETA = 64

# Stage 1: pure fB minimization (wD=0), run to convergence once.
STAGE1_MAXITER = 1000

# Stage 2: discreteness annealing, run as N_ANNEAL_STAGES separate
# L-BFGS-B calls at increasing wD, each warm-started from the last.
N_ANNEAL_STAGES        = 10
STAGE2_MAXITER_PER_SUB = 300
MAX_WD                 = 0.2
WD_RAMP_POWER          = 4

FTOL, GTOL = 1e-12, 1e-10

VOLUME_TARGET_CM3 = 811.25
W_VOLUME_TARGET   = 1.0

B_MAX_T, MU0 = 1.465, 4 * np.pi * 1e-7

# Separate output directory + "_scipy" suffix on every saved file, so
# nothing here ever collides with or overwrites the Adam version's output.
OUTPUT_DIR = Path(__file__).resolve().parent / "pm_opt_custom_loss_scipy_output"

import jax
import jax.numpy as jnp
jax.config.update("jax_enable_x64", True)

from essos.fields import DipoleField
from essos.losses import custom_loss

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)


def load_surface(surf_file, surface_range, nphi, ntheta):
    from simsopt.geo import SurfaceRZFourier
    try:
        surface = SurfaceRZFourier.from_vmec_input(str(surf_file), range=surface_range, nphi=nphi, ntheta=ntheta)
    except Exception:
        surface = SurfaceRZFourier.from_focus(str(surf_file), range=surface_range, nphi=nphi, ntheta=ntheta)
    return surface


def load_coils_essos(coil_file):
    from simsopt.field import Coil, Current
    from simsopt.util.permanent_magnet_helper_functions import read_focus_coils
    from essos.coils import Coils_from_simsopt

    base_curves, base_currents0, ncoils = read_focus_coils(str(coil_file))
    total_current = float(np.sum([c.get_value() for c in base_currents0]))
    all_coils = [Coil(base_curves[i], Current(total_current / ncoils)) for i in range(ncoils)]
    return Coils_from_simsopt(all_coils, nfp=1, stellsym=False)


def compute_Bn_fixed_essos(coils, surf_pts, surf_n):
    from essos.fields import BiotSavart
    field = BiotSavart(coils)
    surf_pts_jax = jnp.asarray(surf_pts, jnp.float64)
    B_at_pts = jax.vmap(field.B)(surf_pts_jax)
    return np.asarray(jnp.sum(B_at_pts * jnp.asarray(surf_n, jnp.float64), axis=1), np.float64)


def load_magnet_grid(mag_file):
    pos_list, mom_list, pho_list, ic_list = [], [], [], []
    with open(str(mag_file), encoding="utf-8") as f:
        for line in f.readlines():
            tokens = line.replace(",", " ").split()
            if len(tokens) < 12:
                continue
            try:
                x, y, z = float(tokens[3]), float(tokens[4]), float(tokens[5])
                ic      = float(tokens[6])
                m0      = float(tokens[7])
                pho     = float(tokens[8])
                az, pol = float(tokens[10]), float(tokens[11])
            except ValueError:
                continue
            pos_list.append((x, y, z))
            mom_list.append((m0*np.cos(az)*np.sin(pol), m0*np.sin(az)*np.sin(pol), m0*np.cos(pol)))
            pho_list.append(pho)
            ic_list.append(ic)
    return (
        np.asarray(pos_list, np.float64),
        np.asarray(mom_list, np.float64),
        np.asarray(pho_list, np.float64),
        np.asarray(ic_list, np.float64),
    )


def write_focus_file(path, positions, moments, pho, header="generated by pm_opt_custom_loss_scipy"):
    n = len(positions)
    m0 = np.linalg.norm(moments, axis=1)
    m0_safe = np.where(m0 > 0, m0, 1.0)
    mhat = moments / m0_safe[:, None]
    pol = np.arccos(np.clip(mhat[:, 2], -1, 1))
    az  = np.arctan2(mhat[:, 1], mhat[:, 0])
    with open(path, "w", encoding="utf-8") as f:
        f.write(f"# {header}\n")
        f.write("# Total number of dipoles\n")
        f.write(f"{n}\n")
        f.write("#coiltype, symmetry,  ox,  oy,  oz,  Ic,  M0,  pho,  Lc,  mp,  mt\n")
        for i in range(n):
            f.write(
                f"2, 2, {positions[i,0]:.8e}, {positions[i,1]:.8e}, {positions[i,2]:.8e}, "
                f"0, {m0[i]:.8e}, {pho[i]:.8e}, 0, {az[i]:.8e}, {pol[i]:.8e}\n"
            )
    print(f"Wrote {n} magnets to {path}")


print(f"--- Loading surface: {SURF_FILE.name} ({SURFACE_NPHI}x{SURFACE_NTHETA}) ---")
surface = load_surface(SURF_FILE, SURFACE_RANGE, SURFACE_NPHI, SURFACE_NTHETA)
nfp, stellsym = int(surface.nfp), bool(surface.stellsym)
print(f"nfp={nfp}  stellsym={stellsym}")

surf_pts = surface.gamma().reshape(-1, 3)
surf_n   = surface.unitnormal().reshape(-1, 3)
surf_n_raw = np.asarray(surface.normal(), np.float64).reshape(-1, 3)
Nnorms = np.linalg.norm(surf_n_raw, axis=1)
n_pts  = len(surf_pts)
area_weight_vec = Nnorms / n_pts

print(f"--- Loading coils: {COIL_FILE.name} ---")
essos_coils = load_coils_essos(COIL_FILE)
Bn_fixed = compute_Bn_fixed_essos(essos_coils, surf_pts, surf_n)

print(f"--- Loading magnet grid: {MAG_FILE.name} ---")
positions, moments_raw, pho_loaded, ic_flags = load_magnet_grid(MAG_FILE)
n_magnets = len(positions)

PORT_MASK = ic_flags < 0.5
n_excluded = int(np.sum(PORT_MASK))
print(f"Ic flag: {n_excluded} excluded sites (Ic=0)")

native_norms = np.linalg.norm(moments_raw, axis=1)
n_zero_moment = int(np.sum(native_norms == 0))
if n_zero_moment > 0:
    print(f"WARNING: {n_zero_moment} zero-moment magnets excluded like Ic=0 sites")
norms_safe = np.where(native_norms > 0, native_norms, 1.0)
M0_SCALE = float(np.mean(native_norms[native_norms > 0])) if np.any(native_norms > 0) else 0.0
orientations = np.where(native_norms[:, None] > 0, moments_raw / norms_safe[:, None], 0.0)
magnet_moments = orientations * M0_SCALE

ZERO_MOMENT_MASK = native_norms == 0
EXCLUDE_MASK = PORT_MASK | ZERO_MOMENT_MASK

M_MAX = B_MAX_T / MU0
volume_per_cell = (M0_SCALE / M_MAX) * 1e6
SYMMETRY_MULTIPLIER = nfp * (2 if stellsym else 1)
print(f"{n_magnets} magnets  |  V_cell={volume_per_cell:.4f} cm3  |  SYMMETRY_MULTIPLIER={SYMMETRY_MULTIPLIER}")

print("--- Build G matrix ---")
t0 = time.time()
dipole_field = DipoleField(
    jnp.asarray(positions, jnp.float32),
    jnp.asarray(magnet_moments, jnp.float32),
    jnp.zeros(n_magnets, jnp.float32),
    nfp=nfp, stellsym=stellsym, scale_factor=1.0,
)
G = np.asarray(dipole_field.compute_interaction_matrix(
    jnp.asarray(surf_pts, jnp.float32), jnp.asarray(surf_n, jnp.float32)), np.float32)
n_nan, n_inf = int(np.sum(np.isnan(G))), int(np.sum(np.isinf(G)))
if n_nan > 0 or n_inf > 0:
    raise RuntimeError(f"G has {n_nan} NaN / {n_inf} Inf values")
print(f"G: {G.shape}  {G.nbytes/1e9:.2f} GB  {time.time()-t0:.1f}s")

f32 = jnp.float32
G_jax  = jnp.asarray(G)
Bn_jax = jnp.asarray(Bn_fixed, f32)
aw_jax = jnp.asarray(area_weight_vec, f32)
vc_jax = f32(volume_per_cell)
Vt_jax = f32(VOLUME_TARGET_CM3)
wVT_jax = f32(W_VOLUME_TARGET)
exclude_mask_jax = jnp.asarray(EXCLUDE_MASK)

fB_gen0 = float(0.5 * np.sum(area_weight_vec * (pho_loaded @ G.T.astype(np.float64) + Bn_fixed) ** 2))
fB_ref = f32(max(fB_gen0, 1e-20))
print(f"fB (loaded FAMUS solution, reference only -- we start from empty) = {fB_gen0:.4e}")


def combined_loss_fn(pho, wD, G, Bn_fix, area_w, fB_ref, vc, Vt, wVT):
    f32 = jnp.float32
    bn = pho @ G.T + Bn_fix
    fB_raw = f32(0.5) * jnp.sum(area_w * bn * bn)
    fB = fB_raw / fB_ref
    abs_p = jnp.sqrt(pho * pho + f32(1e-7))
    fD = jnp.sum(abs_p * (f32(1.0) - abs_p))
    fV = vc * jnp.sum(abs_p)
    fVT = jnp.where(Vt > f32(0), jnp.maximum(fV - Vt, f32(0)), f32(0))
    loss = fB + wD * fD + wVT * fVT

    return loss, (fB_raw, fV, fD)


total_loss = custom_loss(
    combined_loss_fn, "pho", "wD",
    G=G_jax, Bn_fix=Bn_jax, area_w=aw_jax, fB_ref=fB_ref,
    vc=vc_jax, Vt=Vt_jax, wVT=wVT_jax,
)


def loss_and_metrics(pho, wD):
    (loss_val, aux), grad_dict = total_loss.value_and_grad_pytree(
        {"pho": pho, "wD": wD}, has_aux=True
    )
    return loss_val, grad_dict["pho"], aux


def compute_metrics(pho):
    bn = pho @ G_jax.T + Bn_jax
    fB = float(0.5 * jnp.sum(aw_jax * bn * bn))
    abs_p = jnp.sqrt(pho * pho + f32(1e-7))
    fV = float(vc_jax * jnp.sum(abs_p))
    fD = float(jnp.sum(abs_p * (f32(1.0) - abs_p)))
    return fB, fV, fD




def scipy_fun_and_grad(x, wD):
    pho_jax = jnp.asarray(x, dtype=f32)
    val, grad_pho, aux = loss_and_metrics(pho_jax, f32(wD))
    return float(val), np.asarray(grad_pho, dtype=np.float64)


bounds = [(0.0, 0.0) if EXCLUDE_MASK[i] else (-1.0, 1.0) for i in range(n_magnets)]
x0 = np.zeros(n_magnets, dtype=np.float64)
print("*** STARTING FROM EMPTY GRID (pho=0) ***")

hist_fB, hist_fV, hist_fD = [], [], []


def make_callback():
    def callback(xk):
        pho_now = jnp.asarray(xk, dtype=f32)
        fB_now, fV_now, fD_now = compute_metrics(pho_now)
        hist_fB.append(fB_now); hist_fV.append(fV_now); hist_fD.append(fD_now)
    return callback


print(f"\nStage 1 (fB only, wD=0): L-BFGS-B to convergence, maxiter={STAGE1_MAXITER}")
print(f"Stage 2 (discreteness anneal): {N_ANNEAL_STAGES} sub-stages, "
      f"maxiter={STAGE2_MAXITER_PER_SUB} each, wD ramping 0 -> {MAX_WD}")
print(f"Volume target: {VOLUME_TARGET_CM3:.1f} cm3 (unique domain)")

t_start = time.time()

print("\nCompiling / running Stage 1...")
res1 = minimize(
    scipy_fun_and_grad, x0, args=(0.0,), jac=True, method="L-BFGS-B",
    bounds=bounds, callback=make_callback(),
    options={"maxiter": STAGE1_MAXITER, "maxfun": STAGE1_MAXITER * 3, "ftol": FTOL, "gtol": GTOL},
)
print(f"[Stage 1] converged={res1.success}  nit={res1.nit}  loss={res1.fun:.4e}  message={res1.message}")

x_current = res1.x
for k in range(1, N_ANNEAL_STAGES + 1):
    frac = k / N_ANNEAL_STAGES
    wD_val = (frac ** WD_RAMP_POWER) * MAX_WD
    res = minimize(
        scipy_fun_and_grad, x_current, args=(wD_val,), jac=True, method="L-BFGS-B",
        bounds=bounds, callback=make_callback(),
        options={"maxiter": STAGE2_MAXITER_PER_SUB, "maxfun": STAGE2_MAXITER_PER_SUB * 3, "ftol": FTOL, "gtol": GTOL},
    )
    x_current = res.x
    fB_now, fV_now, fD_now = compute_metrics(jnp.asarray(x_current, dtype=f32))
    print(f"[Anneal {k:2d}/{N_ANNEAL_STAGES}] wD={wD_val:.4f}  loss={res.fun:.4e}  "
          f"nit={res.nit}  fB={fB_now:.4e}  fV={fV_now:.1f}  fD={fD_now:.2f}  converged={res.success}")

pho = jnp.asarray(x_current, dtype=f32)

p = np.asarray(pho, np.float64)
p_d = np.sign(p) * (np.abs(p) > 0.5)
p_d = np.where(EXCLUDE_MASK, 0.0, p_d)


def fB64_of(pp):
    r = pp @ G.astype(np.float64).T + Bn_fixed
    return 0.5 * np.sum(area_weight_vec * r * r)


fB_fin = fB64_of(p)
fB_rnd = fB64_of(p_d)
fV_final = float(volume_per_cell * np.sum(np.abs(p_d)))
n_active = int(np.sum(np.abs(p_d) > 0.5))

print(f"\n{'='*70}")
print(f"Done in {time.time()-t_start:.0f}s")
print(f"  Final (smooth):   {fB_fin:.4e}")
print(f"  Hard-rounded:     {fB_rnd:.4e}  fV={fV_final:.1f} cm3 (unique)  = {fV_final*SYMMETRY_MULTIPLIER:.1f} cm3 (full)")
print(f"  Active magnets:   {n_active} / {n_magnets} ({100*n_active/n_magnets:.1f}%)")
print(f"{'='*70}")

np.save(OUTPUT_DIR / "pho_optimized_scipy.npy", p_d)
np.save(OUTPUT_DIR / "pho_continuous_scipy.npy", p)
final_moments = np.where(p_d[:, None] != 0, orientations * M0_SCALE, 0.0) * np.sign(p_d)[:, None]
write_focus_file(OUTPUT_DIR / "pm_optimized_scipy.focus", positions, final_moments, p_d)

np.savez(OUTPUT_DIR / "convergence_history_scipy.npz",
         fB=np.array(hist_fB), fV=np.array(hist_fV), fD=np.array(hist_fD))

import matplotlib.pyplot as plt
fig, axes = plt.subplots(3, 1, figsize=(10, 10), sharex=True)
steps_axis = np.arange(1, len(hist_fB) + 1)
axes[0].plot(steps_axis, hist_fB, color="steelblue"); axes[0].set_yscale("log"); axes[0].set_ylabel("fB")
axes[1].plot(steps_axis, hist_fV, color="seagreen"); axes[1].set_ylabel("fV [cm3]")
axes[2].plot(steps_axis, hist_fD, color="purple"); axes[2].set_yscale("log"); axes[2].set_ylabel("fD"); axes[2].set_xlabel("callback iteration")
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "convergence_history_scipy.png", dpi=200, bbox_inches="tight")
print(f"Saved {OUTPUT_DIR}/convergence_history_scipy.png")
