...
for trace_tolerance_SIMSOPT in trace_tolerance_SIMSOPT_array:
    ...
    time_SIMSOPT_array.append(time()-t1)
    avg_steps_SIMSOPT += sum([len(l) for l in trajectories_SIMSOPT_this_tolerance]) // nparticles
    print(f"  Time for SIMSOPT tracing={time()-t1:.3f}s. Avg num steps={avg_steps_SIMSOPT}")
...
# Trace in ESSOS
num_steps_essos = 1000#int(jnp.mean(jnp.array([len(trajectories_SIMSOPT[0]) for trajectories_SIMSOPT in trajectories_SIMSOPT_array])))
time_essos = jnp.linspace(0, tmax_gc, num_steps_essos)

tracing = Tracing('GuidingCenter', field_essos, 1e-7, timesteps=100, method='Dopri8',
                  stepsize='adaptive', tol_step_size=1e-7, particles=particles)
block_until_ready(tracing.trajectories)

print(f'Tracing ESSOS guiding center with tolerance={trace_tolerance_ESSOS}')
start_time = time()
tracing = Tracing('GuidingCenter', field_essos, tmax_gc, timesteps=num_steps_essos, method='Dopri8',
                  stepsize='adaptive', tol_step_size=trace_tolerance_ESSOS, particles=particles)
block_until_ready(tracing.trajectories)
time_ESSOS = time() - start_time

trajectories_ESSOS = tracing.trajectories
print(f"  Time for ESSOS tracing={time_ESSOS:.3f}s. Num steps={len(trajectories_ESSOS[0])}")
